...
- 예시 2
  ...
  House* myHouse = static_cast<House*>(myPet);    // 컴파일 에러, Animal과 myPet은 상속 관계가 아님
  myHouse->GetAddress();
  ```
...
- static_cast와 비슷하지만 컴파일 중이 아닌 실행 중에 캐스팅이 된다
...
- reinterpret_cast<Cat*> 대신 static_cast<Cat*>
  ...
  - 하지만 reinterpret_cast가 있으면 실수를 했는지 잘 살펴볼 것
